...
driver = webdriver.Safari(service=Service()) # Change this to the appropriate driver
...
class PropertyScraper:
    ...
    def fetch_total_pages(self):
        try:
            ...
            wait(driver, 10).until(EC.presence_of_element_located((By.XPATH, properties_found_xpath)))
            properties_found = driver.find_element(By.XPATH, properties_found_xpath)
        ...
        try:
            for page_number in range(1, self.num_pages + 1): 
                try:
                    ...
                    wait(driver, 10).until(EC.presence_of_element_located((By.XPATH, ul_element_xpath)))
                    ul_element = driver.find_element(By.XPATH, ul_element_xpath)
        ...
        try:
            ...
            for listing_id, url in self.all_property_dict_link.items():
                try:
                    ...
                    wait(driver, 10).until(EC.presence_of_element_located((By.XPATH, type_xpath)))
                    type = driver.find_element(By.XPATH, type_xpath).text.strip()
                    try:
                    ...
                    phone_numbers = [phone_number_element.find_element(By.TAG_NAME, 'span').text.strip() for phone_number_element in driver.find_elements(By.XPATH, phone_numbers_xpath)]
                    phone_numbers = list(set(phone_numbers))
                    phoneNumbers = ', '.join(phone_numbers)
                    
                    message_xpath = "//p[@class='listing-contact__no-solicitations-body']"
                    try:
                        message = driver.find_element(By.XPATH, message_xpath)
                    ...
                    characteristics_viewports = driver.find_elements(By.XPATH, characteristics_viewports_xpath)
